...
const Title = () => {
  ...
  const [subtitles,setSubtitle] = useState<subtitle[]>([])
  ...
  return(
    <div>
      ...
      {title && (
        <>
        <h2 className="pd-4 text-4xl font-bold text-center">{title.title}</h2>
        <p className="pb-4 text-2xl font-semibold text-center">{title.author}</p>
        </>
      )}
      {title && user && (
        <SubtitleList 
