...
export const SubtitleCard = (props: Props) => {
  ...
  const [volume,setVolume] = useState<string>(props.subtitle.volume.toString())
  const [isbn,setIsbn] = useState<string>(props.subtitle.isbn.toString())
  const [possession,setPossession] = useState<boolean>(props.subtitle.possession)
  ...
  const handleSetThumbnail = useCallback(async()=> {
    ...
    const { error } = await (await client.from("manga_title")
    .upsert(title))
    if(error){
  ...
  const handleSave = useCallback(async()=>{
    ...
    if(openbd[0] == null){
      alert("Invaid ISBN number Please check")
      return
    }
    ...
    <>
      <div className="p-2 border cursor-pointer" onClick={openModal}>
        ...
        <Transition appear show={isOpen} as={Fragment}>
              <Dialog
                as="div"
                className="flex inset-0 z-10 overflow-y-auto"
                onClose={closeModal}
              >
                <div className="min-h-screen px-4 text-center border-2">
                  <span className="inline-block h-screen aligin-middle" area-hidden="true">&#8203;</span>
                  <Transition.Child
                    as={Fragment}
                    ...
                    <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform border border-gray-300 shadow-xl bg-gray-50 rounded-xl">
                              ...
                              {({open})=>(
                                <>
                                  ...
                                  <Disclosure.Panel className="p-4 text-gray-500 text-md">
                                      <Button block onClick={handleRemove} icon={<IconTrash2 />}>
                                        REMOVE
                                      </Button>
                                  </Disclosure.Panel>
                                </>
                      ...
                      <div className="flex justify-center mt-4">
                        <div className="w-32 p-2">
                          <Button type="default" size="large" icon={<IconX />} onClick={closeModal}>
                            Cancel
                          </Button>
                        </div>
                        <div className="w-32 p-2">
