  ...
  const handleSave = useCallback(async () => {
    ...
    if (id) {
      const { error } = await client.from("profile").upsert({
        ...
        user_id: uid,
        name: editName,
        icon: iconExists || iconChaanged,
      });
      if (error) {
        alert("Failed: Updata Profile");
      }
    } else {
      const { error } = await client
        .from("profile")
        .insert({ user_id: uid, name: editName, icon: iconChaanged });
      if (error) {
      ...
      <Transition appear show={isOpen} as={Fragment}>
        ...
        >
          ...
          >
            ...
            >
              <div className="inline-block w-full max-w-md p-6 py-8 overflow-hidden text-left align-middle transition-all transform border border-gray-300 shadow-xl bg-gray-50 rounded-xl">
                <Dialog.Title
                  as="h3"
